...
func (c *gcsCore) cleanupContainer(containerEntry *containerCacheEntry) error {
	...
	for _, disk := range containerEntry.MappedVirtualDisks {
		if !disk.AttachOnly {
			if err := storage.UnmountPath(context.Background(), disk.ContainerPath, false); err != nil {
				logrus.Warn(err)
				if errToReturn == nil {
					errToReturn = err
				}
	...
	for _, directory := range containerEntry.MappedDirectories {
		if err := storage.UnmountPath(context.Background(), directory.ContainerPath, false); err != nil {
			logrus.Warn(err)
			if errToReturn == nil {
				errToReturn = err
			}
